...
/**
 * Structural view of the media element the adapter drives. MissAV's player is
 * a plain HTMLVideoElement, so the shape matches the Bilibili/YouTube
 * adapters' minimal structural interface: tests pass a fake object without
 * casts while real elements still fit.
 */
export interface MissavMediaElement {
...
export type MissavMediaCollection = ArrayLike<MissavMediaElement> & Iterable<MissavMediaElement>;
...
/** Host pattern for MissAV sites: `missav.live` itself or any `*.missav.live` subdomain. */
const MISSAV_HOST_PATTERN = /(^|\.)missav\.live$/;

/** Canonical base every MissAV video identity collapses onto, mirror-independent. */
const MISSAV_CANONICAL_BASE = 'https://missav.live/';

/**
 * Full-URL pattern for MissAV VIDEO pages: `/<locale>/<dvd-id>` with an
 * optional `/dm<N>/` mirror segment in front, e.g. `/en/mxgs-1440` or
 * `/dm1/en/mxgs-1440`. The locale is a short alpha token (`en`, `ja`, `zh`,
 * optionally `zh-cn`); the video id is deliberately CONSERVATIVE: it must be
 * a hyphenated id (`mxgs-1440`) so homepage, locale-only, and non-hyphenated
 * paths never resolve to a video. The serialized registry `AdapterUrlRule`
 * uses `source`, and the adapter's identity guard matches the same shape, so
 * the registry rule and the identity policy cannot drift.
 */
export const MISSAV_VIDEO_URL_PATTERN = /^https?:\/\/(?:[^/]+\.)?missav\.live\/(?:dm\d+\/)?([a-z]{2}(?:-[a-z]{2})?)\/([a-z0-9]+-[a-z0-9]+)\/?(?=[?#]|$)/;

export class MissavAdapter implements ResourceAdapter {
  readonly adapterId = 'missav';
  private readonly page: MissavPage;
  private target: MissavMediaElement | undefined;
  private buffering = false;

  constructor(page?: MissavPage) {
    // Resolve the browser globals lazily so `new MissavAdapter()` never crashes in
  ...
  identifyResource(): ResourceIdentity {
    ...
    if (typeof href !== 'string' || href.length === 0) {
      throw new AdapterSiteError('browser-required', 'MissavAdapter needs a page location to identify the resource');
    }
    let url: URL;
    try {
      url = new URL(href);
    } catch {
      throw new AdapterSiteError('invalid-url', `Cannot parse MissAV page URL: ${href}`);
    }
    if (!MISSAV_HOST_PATTERN.test(url.hostname)) {
      ...
      throw new AdapterSiteError('not-bilibili', `Page host ${url.hostname} is not a MissAV site`);
    }
    // Match against the normalized serialization so host case is handled and the
    // pattern is the single source of truth for the accepted video shapes.
    const match = MISSAV_VIDEO_URL_PATTERN.exec(url.href);
    if (match === null) {
      throw new AdapterSiteError('not-bilibili', `Page URL ${href} is not a MissAV video page`);
    }
    const locale = match[1] as string;
    const dvdId = match[2] as string;
    // Canonical identity: rebuilt from locale + dvd id alone, so the mirror
    // segment (`/dmN/`), subdomain, query parameters, fragment and trailing
    // slash never change the resource. The dvd id is the site-native
    // resourceId, mirroring Bilibili's BV id and YouTube's video id.
    return {
      adapterId: this.adapterId,
      canonicalUrl: `${MISSAV_CANONICAL_BASE}${locale}/${dvdId}`,
      resourceId: dvdId,
    };
  }
  ...
  selectTarget(): void {
    ...
    if (document === undefined || typeof document.querySelectorAll !== 'function') {
      throw new AdapterSiteError('browser-required', 'MissavAdapter needs a page document to find a video');
    }
    const candidates = Array.from(document.querySelectorAll('video'))
      .filter((candidate) => candidate.readyState > 0 || Number.isFinite(candidate.duration));
    if (candidates.length === 0) {
      throw new AdapterSiteError('no-media', 'No playable MissAV video found on this page');
    }
  ...
  private requireTarget(): MissavMediaElement {
    if (this.target === undefined) this.selectTarget();
    if (this.target === undefined) {
      throw new AdapterSiteError('no-media', 'No MissAV video target is available');
    }
    return this.target;
  }
   ...
   */
  private phaseFor(target: MissavMediaElement): MediaPhase {
  ...
  private readStateOf(target: MissavMediaElement): LocalPlaybackState {
    return {
  ...
  private applyPhase(target: MissavMediaElement, phase: MediaPhase): Promise<void> | void {
    switch (phase) {
      case 'playing':
        return target.play();
      case 'paused':
  ...
  private async waitForSeekSettled(target: MissavMediaElement): Promise<void> {
  ...
  private visibleArea(candidate: MissavMediaElement): number {
  ...
  private bestEffortState(target: MissavMediaElement): LocalPlaybackState {
    try {
...
/**
 * Registry contract for the MissAV syncer: an http(s) page on missav.live
 * or a `*.missav.live` subdomain whose path is `/<locale>/<dvd-id>` with an
 * optional `/dm<N>/` mirror segment. Other MissAV pages do not resolve to
 * this syncer; the adapter's `identifyResource` enforces the same video-path
 * rule at identity time, mirroring the serialized `urlRule`.
 */
export const missavRegistration: SyncerRegistration = {
  adapterId: 'missav',
  name: 'MissAV',
  domain: 'missav.live',
  // Serialized full-URL rule (source + flags survive JSON / structured clone):
  // only `/locale/hyphenated-id` and `/dmN/locale/hyphenated-id` pages match,
  // so `resolve` never hands non-video MissAV pages to this syncer. The source
  // is shared with the identity guard, so the registry rule and identity
  // policy cannot drift.
  urlRule: { source: MISSAV_VIDEO_URL_PATTERN.source },
  create: (page: AdapterPage) => {
    const document = page.document as MissavDocument | undefined;
    return new MissavAdapter(
      document === undefined
