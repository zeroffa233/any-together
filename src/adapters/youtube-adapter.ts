...
/**
 * Structural view of the media element the adapter drives. YouTube's player is
 * a plain HTMLVideoElement, so the shape matches the Bilibili adapter's
 * minimal structural interface: tests pass a fake object without casts while
 * real elements still fit.
 */
export interface YoutubeMediaElement {
...
export type YoutubeMediaCollection = ArrayLike<YoutubeMediaElement> & Iterable<YoutubeMediaElement>;
...
/** Host pattern for YouTube sites: `youtube.com` itself or any `*.youtube.com` subdomain. */
const YOUTUBE_HOST_PATTERN = /(^|\.)youtube\.com$/;

/** Canonical base every YouTube watch identity collapses onto, host-independent. */
const YOUTUBE_CANONICAL_BASE = 'https://www.youtube.com/watch?v=';

/**
 * Full-URL pattern for YouTube WATCH pages: `/watch` with a non-empty `v=`
 * query parameter in any position (`?v=…` or `?a=1&v=…`). The serialized
 * registry `AdapterUrlRule` uses `source`, and the adapter's identity guard
 * is written against the same shape, so the registry rule and the identity
 * policy cannot drift. Non-watch YouTube pages (homepage, shorts, feeds) and
 * watch pages without a `v` parameter are deliberately excluded.
 */
export const YOUTUBE_WATCH_URL_PATTERN = /^https?:\/\/(?:[^/]+\.)?youtube\.com\/watch\?(?:[^#]*&)?v=[^&#]+/;

export class YoutubeAdapter implements ResourceAdapter {
  readonly adapterId = 'youtube';
  private readonly page: YoutubePage;
  private target: YoutubeMediaElement | undefined;
  private buffering = false;

  constructor(page?: YoutubePage) {
    // Resolve the browser globals lazily so `new YoutubeAdapter()` never crashes in
  ...
  identifyResource(): ResourceIdentity {
    ...
    if (typeof href !== 'string' || href.length === 0) {
      throw new AdapterSiteError('browser-required', 'YoutubeAdapter needs a page location to identify the resource');
    }
    let url: URL;
    try {
      url = new URL(href);
    } catch {
      throw new AdapterSiteError('invalid-url', `Cannot parse YouTube page URL: ${href}`);
    }
    if (!YOUTUBE_HOST_PATTERN.test(url.hostname)) {
      ...
      throw new AdapterSiteError('not-bilibili', `Page host ${url.hostname} is not a YouTube site`);
    }
    if (url.pathname !== '/watch') {
      throw new AdapterSiteError('not-bilibili', `Page URL ${href} is not a YouTube watch page`);
    }
    const videoId = url.searchParams.get('v');
    if (typeof videoId !== 'string' || videoId.length === 0) {
      throw new AdapterSiteError('not-bilibili', `Page URL ${href} has no video id`);
    }
    // Canonical identity: rebuilt from the video id alone, so query order,
    // extra parameters (list/t/...) and the fragment never change the
    // resource, and every host (youtube.com, www, m, music) collapses onto
    // one canonical watch URL. The video id is also the site-native
    // resourceId, mirroring Bilibili's BV id.
    return {
      adapterId: this.adapterId,
      canonicalUrl: `${YOUTUBE_CANONICAL_BASE}${videoId}`,
      resourceId: videoId,
    };
  }
  ...
  selectTarget(): void {
    ...
    if (document === undefined || typeof document.querySelectorAll !== 'function') {
      throw new AdapterSiteError('browser-required', 'YoutubeAdapter needs a page document to find a video');
    }
    const candidates = Array.from(document.querySelectorAll('video'))
      .filter((candidate) => candidate.readyState > 0 || Number.isFinite(candidate.duration));
    if (candidates.length === 0) {
      throw new AdapterSiteError('no-media', 'No playable YouTube video found on this page');
    }
  ...
  async applyState(targetState: AdapterTargetState): Promise<AdapterApplyResult> {
    let target: YoutubeMediaElement;
    try {
  ...
  private requireTarget(): YoutubeMediaElement {
    if (this.target === undefined) this.selectTarget();
    if (this.target === undefined) {
      throw new AdapterSiteError('no-media', 'No YouTube video target is available');
    }
    return this.target;
  }
   ...
   */
  private phaseFor(target: YoutubeMediaElement): MediaPhase {
  ...
  private readStateOf(target: YoutubeMediaElement): LocalPlaybackState {
    return {
  ...
  private applyPhase(target: YoutubeMediaElement, phase: MediaPhase): Promise<void> | void {
    switch (phase) {
      case 'playing':
        return target.play();
      case 'paused':
  ...
  private async waitForSeekSettled(target: YoutubeMediaElement): Promise<void> {
  ...
  private visibleArea(candidate: YoutubeMediaElement): number {
  ...
  private bestEffortState(target: YoutubeMediaElement): LocalPlaybackState {
    try {
...
/**
 * Registry contract for the YouTube syncer: an http(s) page on youtube.com
 * or a *.youtube.com subdomain whose path is `/watch` with a non-empty `v=`
 * query parameter in any position. Other YouTube pages (homepage, shorts,
 * feeds, watch without v) do not resolve to this syncer; the adapter's
 * `identifyResource` enforces the same watch-page rule at identity time,
 * so the registry rule and the identity policy cannot drift.
 */
export const youtubeRegistration: SyncerRegistration = {
  adapterId: 'youtube',
  name: 'YouTube',
  domain: 'youtube.com',
  // Serialized full-URL rule (source + flags survive JSON / structured clone):
  // only `/watch` pages with a non-empty `v=` query parameter match, so
  // `resolve` never hands non-watch YouTube pages to this syncer.
  urlRule: { source: YOUTUBE_WATCH_URL_PATTERN.source },
  create: (page: AdapterPage) => {
    const document = page.document as YoutubeDocument | undefined;
