...
- **验证现状**：`npm test` 全绿（230 项：playback-state、sync-items、session-authority、dynamic-resource、local-video-resource、sync-items 集成、local-media-server、五个站点适配器、local-video-extension 等套件），`smoke:lan` 与 `smoke:process` 通过，扩展 JS/Manifest 静态检查通过；真实 Bilibili 页面（BV1mkgw6mEQt）已验证媒体元素可控性（记录于需求规格 §14）。真实浏览器 / 跨设备实机验证尚未执行（见各 Phase 未闭环项）。
- **未验证缺口**：真实双设备局域网端到端（L3，媒体与本地视频）、Chrome 扩展真实加载端到端、arXiv PDF 页面注入可行性、macOS 防火墙/Windows 实机行为——这些在各 Phase 验收中补齐或明确豁免记录。
...
3. 从机收到 URL，扩展在 popup 请求当前 HTTP origin 权限；用户允许后动态注入通用媒体 content script 并打开该 URL。
4. 播放/暂停/拖动/倍速全部复用网页同步语义；拖动依赖服务端 Range 请求。

### 6.3 核心模型变化

- **本地媒体服务** `[已落地]`：伴随进程新增 tokenized 单文件服务，支持 HTTP Range、正确 Content-Type、流式传输；只在明确共享时监听局域网接口。
- **安全模型** `[已落地，实机待验证]`：高熵 URL token、停止吊销、无共享不监听；扩展使用 `scripting` + `optional_host_permissions` 请求当前 `origin/*`，不使用 `<all_urls>` 静态注入。
- **资源身份** `[已落地]`：浏览器 `identity.js` 注册动态 IPv4/localhost `local-video` URL，background 与 Node 权威状态复用同一 token URL。

- **会话模型**：完全复用，零改动——服务与扩展接入是本阶段新增组件。
...
- CLI 分享、资源身份、Range 服务和动态扩展权限/注入代码已落地；Node 集成/扩展注册测试全绿。
- **[未验证]** 真实 Chrome 两端共享本地 mp4，play/pause/seek（含未缓冲区间）/rate 满足既有收敛基线。
- Range 请求验证（seek 后秒级开始播放，不整文件下载）。
- 无令牌请求被拒绝；未共享时不监听局域网媒体端口。
- **[未验证]** 两端编解码器不一致时显式报错而非黑屏挂起。
- 既有测试全绿 + 媒体服务与扩展注册测试。
...
- issue：真实 Chrome/双设备共享验证（含权限弹窗、防火墙、编解码器记录）。
